...
# Run unit tests (VocabSparkTests: SessionDeckEngine + SRSEngine)
xcodebuild test -project VocabSpark.xcodeproj -scheme VocabSpark -destination 'platform=iOS Simulator,name=iPad Air 13-inch (M4)'
```
...
- **Card UI**: `SessionCardView` is shared by both session modes. Content is passed by value and each advance gets a fresh `.id(engine.generation)` so an outgoing card never shows the next card's answer. Swipe on a revealed card: right = knew it, left = again (physical direction, also in RTL).
...
- iPad keyboard shortcuts: Space = reveal, 1 = Nochmal, 2 = Gewusst/Kann ich
